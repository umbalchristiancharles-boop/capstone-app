...
for (let i=0;i<lines.length;i++){
  ...
  while ((m = re.exec(line)) !== null) {
    ...
    if (isClose) {
      console.log(`FOUND closing </${tag}> at line ${i+1}`);
      if (stack.length===0) {
        console.error(`Unexpected closing </${tag}> at line ${i+1}`);
        process.exit(3);
      }
      const top = stack[stack.length-1];
      console.log(` TOP of stack is <${top.tag}> opened at ${top.line}`);
      if (top.tag === tag) {
        console.log(`  Pop <${tag}>`);
        stack.pop();
      } else {
      ...
      console.log(`FOUND opening <${tag}> at line ${i+1} ${selfClose?'(self-close)':''}`);
